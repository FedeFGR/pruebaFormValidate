function Validar(){
	...
	if(cadena.length==0 || cadena.length>50)
		msg=msg+"\nEl Nombre esta vacío o tiene mas de 50 letras";
	for(i=0;i<cadena.length;i++)
		if(!(cadena[i]>='a' && cadena[i]<='z' || cadena[i]>='A' && cadena[i]<='Z'))
		{	msg=msg+"\nEl Nombre tiene caracteres que no son letras";
			break;
		}
	//verificar Apellido
	cadena=document.getElementById("apellido").value;
	if(cadena.length==0 || cadena.length>30)
		msg=msg+"\nEl Apellido esta vacío o tiene mas de 30 letras";
	for(i=0;i<cadena.length;i++)
		if(!(cadena[i]>='a' && cadena[i]<='z' || cadena[i]>='A' && cadena[i]<='Z'))
		{	msg=msg+"\nEl Apellido tiene caracteres que no son letras";
			break;
		}
	//verificar email
